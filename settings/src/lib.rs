...
pub struct Setting<T: 'static, R = T> {
...
impl<T: 'static, R> Borrow<str> for Setting<T, R> {
    fn borrow(&self) -> &str {
        self.path()
    }
}

impl<T: 'static, R> AsRef<str> for Setting<T, R> {
    fn as_ref(&self) -> &str {
        self.path()
    }
}

impl<T: 'static, R> Eq for Setting<T, R> {}

impl<T: 'static, R, S: AsRef<str>> PartialEq<S> for Setting<T, R> {
    fn eq(&self, other: &S) -> bool {
        self.path().eq(other.as_ref())
    }
}

impl<T: 'static, R, S: AsRef<str>> PartialOrd<S> for Setting<T, R> {
    fn partial_cmp(&self, other: &S) -> Option<::core::cmp::Ordering> {
        Some(self.path().cmp(other.as_ref()))
    }
}

impl<T: 'static, R> Ord for Setting<T, R> {
    fn cmp(&self, other: &Self) -> ::core::cmp::Ordering {
        self.path().cmp(other.path())
    }
}

impl<T: 'static, R> Hash for Setting<T, R> {
    fn hash<H: ::core::hash::Hasher>(&self, state: &mut H) {
        self.path().hash(state);
    }
}

impl<T: 'static, R> Clone for Setting<T, R> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<T: 'static, R> Copy for Setting<T, R> {}

impl<T: 'static, R> Debug for Setting<T, R> {
